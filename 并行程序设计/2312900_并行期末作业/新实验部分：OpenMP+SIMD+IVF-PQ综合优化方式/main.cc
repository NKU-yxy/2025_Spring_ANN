...
// ------综合实验------------------//
#include "ivfpq_search_combined.h"
...
*/
   ...
   // IVFPQIndex iiidx = build_ivf_pq_index(base_vec, nlist, M, Ks, dim);
    // 保存索引
    //save_index(iiidx,"files/combined_index04");

    // ------------------若需要调用以前的那就直接加载就好了~--------------------

    // 加载索引
    IVFPQIndex my_index = load_index("files/combined_index04");
    ...
    for(int i = 0; i < test_number; ++i) {
       ...
       // auto res = openmp_search(my_index,query_vec,k,nprobe);
        ...
        // 综合实验部分
        auto res = ivf_pq_search_parallel(my_index,query_vec,k,nprobe);

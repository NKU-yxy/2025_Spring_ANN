...
struct MinHeapComparator {
    bool operator()(const std::pair<float, int>& a, const std::pair<float, int>& b) const {
        return a.first > b.first; // 按距离升序，构建最小堆
    }
};

std::priority_queue<std::pair<float, int>, std::vector<std::pair<float, int>>, MinHeapComparator>
openmp_ivf_pq_search(const IVFPQIndex& index, const std::vector<float>& query, int topk, int nprobe) {
    int dim = index.dim;
    int M = index.M;
    int Ks = index.Ks;
    int sub_dim = dim / M;
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < nprobe; ++i) {
            ...
            const auto& centroid = index.coarse_centroids[cid];
            const auto& invlist = index.inverted_lists.at(cid);
            ...
            for (size_t j = 0; j < invlist.pq_codes.size(); ++j) {
                ...
                for (int m = 0; m < M; ++m) {
                    ...
                    const auto& codeword = index.pq_codebooks[m][code];
                    pq_dist += l2_distance_sq(residual_subqueries[m], codeword);
                }
                local_candidates.emplace_back(pq_dist, invlist.ids[j]); // 参数顺序为 (距离, id)
            }
        }
    ...
    // 3. 排序并取 topk
    std::partial_sort(
        candidates.begin(), candidates.begin() + std::min(topk, (int)candidates.size()),
        candidates.end(), [](const std::pair<float, int>& a, const std::pair<float, int>& b) { return a.first < b.first; }

    );
    candidates.resize(std::min(topk, (int)candidates.size()));

    // 4. 构建堆返回
    std::priority_queue<std::pair<float, int>, std::vector<std::pair<float, int>>, MinHeapComparator> heap;
    for (const auto& c : candidates) {
        heap.emplace(c.first, c.second);
    }
    return heap;
}

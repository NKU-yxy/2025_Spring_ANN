...
/*
写于MPI实验:
因为我还没在服务器上测试过，所以若要运行验证的话 需要先创建对应的索引（创建的代码我已经写好了）
并且创建完后会自动运行 所以需要多等一会儿就能看见了
别的MPI的处理我也处理完了 运行一次应该就能看见结果了
*/
...
int main(int argc, char *argv[])
{
   // MPI_Init(&argc,&argv);
    ...
    // 初始化 GPU 内存（只需一次）
   // init_gpu_memory(base, base_number, vecdim);

    std::vector<SearchResult> results;
    results.resize(test_number);

    // 预热 GPU（避免首次启动延迟）
   // flat_search_gpu(test_query, 100, vecdim, k);
...
*/
   ...
   // IVFPQIndex iiidx = build_ivf_pq_index(base_vec, nlist, M, Ks, dim);
    // 保存索引
   // save_index(iiidx,"files/mpi_index");

    // ------------------若需要调用以前的那就直接加载就好了~--------------------

    // 加载索引
   // IVFPQIndex my_index = load_index("files/mpi_index");
    ...
    for(int i = 0; i < test_number; ++i) {
       ...
       // auto res = openmp_search(my_index,query_vec,k,nprobe);
        ...
        // mpi实验部分
       // auto res = ivf_pq_search_mpi(my_index, query_vec, k, nprobe);

       // GPU实验部分
       // auto res = flat_search_gpu(test_query + i * vecdim, base_number, vecdim, k);

       // V1-V4
      // auto res = flat_search_gpu(base, test_query + i*vecdim, base_number, vecdim, k);

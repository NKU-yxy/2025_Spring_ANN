...
// 采用SIMD方式加速欧氏距离平方 (适用于 float32)
#ifdef __ARM_NEON
inline float l2_distance_sq_neon(const float* a, const float* b, int dim) {
...
ivf_pq_search(const IVFPQIndex& index, const std::vector<float>& query, int topk, int nprobe) {
    ...
    for (int i = 0; i < nprobe; ++i) {
        ...
        for (int j = 0; j < invlist.pq_codes.size(); ++j) {
            float pq_dist = 0.0f;
            const auto& codes = invlist.pq_codes[j];
            for (int m = 0; m < M; ++m) {
                pq_dist += l2_distance_sq(residual_subqueries[m], index.pq_codebooks[m][codes[m]]);
            }
...
// 并行化后的search:
// 并行化后的 ivf_pq_search
// 并行化 + 局部堆优化后的 ivf_pq_search
inline std::priority_queue<std::pair<float, int>, std::vector<std::pair<float, int>>, std::less<std::pair<float, int>>>
ivf_pq_search_parallel(const IVFPQIndex& index, const std::vector<float>& query, int topk, int nprobe) {
    const int dim = index.dim;
    const int M = index.M;
    const int Ks = index.Ks;
    const int sub_dim = dim / M;

    std::vector<std::pair<int, float>> coarse_dists(index.coarse_centroids.size());

#pragma omp parallel for
    for (int i = 0; i < static_cast<int>(index.coarse_centroids.size()); ++i) {
        coarse_dists[i] = std::make_pair(i, l2_distance_sq_neon(query.data(), index.coarse_centroids[i].data(), dim));
    }
    ...
    // 使用最大堆以便在 size 超过 topk 时弹出当前"最差"(距离最大) 的元素
    std::vector<std::priority_queue<std::pair<float, int>, std::vector<std::pair<float, int>>, std::less<std::pair<float, int>>>> local_heaps;
    int thread_num = omp_get_max_threads();
    local_heaps.resize(thread_num);
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < nprobe; ++i) {
        ...
        // 1) 先计算 residual_subqueries
        std::vector<std::vector<float>> residual_subqueries(M, std::vector<float>(sub_dim));
        for (int m = 0; m < M; ++m)
            for (int d = 0; d < sub_dim; ++d)
                residual_subqueries[m][d] = query[m * sub_dim + d] - centroid[m * sub_dim + d];

        // 2) 为当前 coarse centroid 预计算查询 LUT (M x Ks)
        std::vector<std::vector<float>> dist_lut(M, std::vector<float>(Ks));
        for (int m = 0; m < M; ++m) {
            for (int k = 0; k < Ks; ++k) {
                dist_lut[m][k] = l2_distance_sq_neon(residual_subqueries[m].data(), index.pq_codebooks[m][k].data(), sub_dim);
            }
        }

        // 3) 遍历倒排表，利用 LUT 快速累加距离
        for (int j = 0; j < static_cast<int>(invlist.pq_codes.size()); ++j) {
            float pq_dist = 0.0f;
            const auto& codes = invlist.pq_codes[j];

            // 当前堆中距离阈值（最大距离）
            float cur_threshold = (static_cast<int>(local_heap.size()) == topk) ? local_heap.top().first : std::numeric_limits<float>::infinity();

            for (int m = 0; m < M; ++m) {
                pq_dist += dist_lut[m][codes[m]];
                // Early-exit：一旦部分和已超过阈值，就放弃该向量
                if (pq_dist > cur_threshold) {
                    break;
                }
            }

            if (pq_dist <= cur_threshold) {
                local_heap.emplace(pq_dist, invlist.ids[j]);
                if (static_cast<int>(local_heap.size()) > topk) local_heap.pop();
            }
        }
    }

    std::priority_queue<std::pair<float, int>, std::vector<std::pair<float, int>>, std::less<std::pair<float, int>>> result_heap;
    for (auto& heap : local_heaps) {
        while (!heap.empty()) {
            result_heap.push(heap.top());
            if (static_cast<int>(result_heap.size()) > topk) result_heap.pop();
            heap.pop();
        }
    }
